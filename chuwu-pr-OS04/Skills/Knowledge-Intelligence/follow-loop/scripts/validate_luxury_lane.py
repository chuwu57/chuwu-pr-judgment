#!/usr/bin/env python3
"""Validate Follow Loop Luxury Follow assets."""
...
ASSET_ROOT = Path("Domains/Business/Luxury")
EVIDENCE_ROOT = Path("Domains/PR/60-References/Evidence/Luxury")
RELATIONS = {"新分支", "强化", "细化", "修正", "反转", "从观点到行动", "当前信号"}
CONTENT_TYPES = {"业绩", "战略", "品牌", "组织", "研究", "信息"}
    ...
    for profile in profiles:
        ...
        for line in section.splitlines():
            ...
            if len(cells) != 5 or cells[2] not in CONTENT_TYPES or cells[3] not in RELATIONS:
                errors.append(f"奢侈品更新记录枚举非法：{profile.name} {line}")
            if not re.search(r"\[原文\]\(https?://", cells[-1]):
                errors.append(f"奢侈品更新记录未直链原文：{profile.name} {line}")
        urls = [normalized_url(url) for url in re.findall(r"^- 原始链接：\s*(https?://\S+)", evidence_text, re.M)]
        if not urls:
            errors.append(f"奢侈品Evidence没有原始链接：{evidence.name}")
        if len(urls) != len(set(urls)):
            errors.append(f"奢侈品Evidence存在规范化重复URL：{evidence.name}")
    return errors
...
def main() -> int:
    ...
    if errors:
        print("FAIL: Follow Loop Luxury Follow")
        for error in errors:
            print(f"- {error}")
        return 1
    print("PASS: Follow Loop Luxury Follow")
    return 0
